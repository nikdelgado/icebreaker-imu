...
assign spi_mosi = spi_shift_register[23];
...
function [7:0] get_tx_byte (input [2:0] idx);
    case (idx)
        3'd0: get_tx_byte = hex_to_char(spi_result[15:12]);
        3'd1: get_tx_byte = hex_to_char(spi_result[11:8]);
        3'd2: get_tx_byte = hex_to_char(spi_result[7:4]);
        3'd3: get_tx_byte = hex_to_char(spi_result[3:0]);
        3'd4: get_tx_byte = 8'h0D;
...
always @(posedge CLK) begin
    ...
    if (clock_divider == 4) begin
        ...
        case (state)
            IDLE: begin
                if (!spi_woken) begin
                    spi_shift_register <= 24'h060100; // write PWR_MGMT_1 = 0x01
                end else begin
                    spi_shift_register <= 24'hAD0000; // read 
                end
                ...
                if (spi_sclk == 0) begin
                    ...
                    spi_shift_register <= {spi_shift_register[22:0], spi_rx_bit};
                    spi_bit_counter <= spi_bit_counter + 1;

                    if (spi_bit_counter == 23) begin
                        state <= DONE;
                    end
                end
                ...
                spi_cs <= 1;
                spi_result <= spi_shift_register[15:0];
                spi_woken <= 1;
                state <= IDLE;
            end
        endcase
